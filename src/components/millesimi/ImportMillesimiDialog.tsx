...
export function ImportMillesimiDialog({ open, onOpenChange, buildings, onSuccess }: ImportMillesimiDialogProps) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${supabaseUrl}/functions/v1/parse-millesimi-file`, {
        method: "POST",
        ...
        {step === "upload" && (
          <div className="space-y-4 py-4">
            <div className="flex items-center gap-2 rounded-md bg-primary/10 p-3 text-sm">
              ...
              <span>
                {t("millesimi.import.targetBuilding")}: <strong>{selectedBuilding?.name}</strong>
              </span>
            </div>
            <div
            ...
            <div className="rounded-lg border overflow-hidden">
              <Table>
                ...
                <TableBody>
                  {units.map((unit, idx) => {
                    const pct =
                      totalMillesimi > 0 ? ((unit.millesimi_value / totalMillesimi) * 100).toFixed(2) : "0.00";
                    return (
                      <TableRow key={idx}>
                        <TableCell>
                          <Input
                ...
                <TableFooter>
                  <TableRow>
                    <TableCell colSpan={2} className="font-medium">
                      {t("millesimi.import.total")}
                    </TableCell>
                    <TableCell className="text-right font-medium">{totalMillesimi}</TableCell>
                    <TableCell className="text-right font-medium">{totalMillesimi > 0 ? "100.00%" : "0.00%"}</TableCell>
                    <TableCell colSpan={3}>
              ...
              <CardContent className="pt-6 space-y-4">
                ...
                <div className="text-sm text-muted-foreground space-y-1">
                  ...
                  {units.some((u) => u.owner_name) && (
                    <p>
                      • {t("millesimi.import.willCreateResidents", { count: units.filter((u) => u.owner_name).length })}
                    </p>
                  )}
                </div>
              </CardContent>
